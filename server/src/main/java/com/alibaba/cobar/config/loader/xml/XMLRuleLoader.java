...
public class XMLRuleLoader {
    ...
    public XMLRuleLoader(String ruleFile) {
        this.rules = new HashSet<RuleConfig>();	/* 全部rule列表 */
        this.tableRules = new HashMap<String, TableRuleConfig>();	/* ruleName -> rule列表*/
        this.functions = new HashMap<String, RuleAlgorithm>();	/* funName -> func */
    ...
    private RuleAlgorithm createFunction(String name, String clazz) throws ClassNotFoundException,
        ...
        return (RuleAlgorithm) constructor.newInstance(name);	/* 调用只有一个string参数的构造函数 */
    }
