...
public class MySQLExprParser extends MySQLParser {
    ...
    private Expression primaryExpression(final String consumed, String consumedUp) throws SQLSyntaxErrorException {
        ...
        case PUNC_LEFT_PAREN:
            lexer.nextToken();
            if (lexer.token() == KW_SELECT) {	/* 括号中的select语句 */
                tempExpr = subQuery();
                match(PUNC_RIGHT_PAREN);
                return tempExpr;
            }
     ...
     * <code>null</code>
		 * 识别一个identifier之后的单元,包括函数
		 * 识别一些函数的，已经连个联系字符串的 如 _latin1 xxx  和 add(a,b)
     */
    private Expression startedFromIdentifier(final String consumed, String consumedUp) throws SQLSyntaxErrorException {
        ...
        case PUNC_DOT: /*  .*  .x.*  */
            ...
            return tempExpr;
        case LITERAL_BIT: /* TODO */
            if (consumed.charAt(0) != '_') {	/* identify bit */
                return new Identifier(null, consumed, consumedUp).setCacheEvalRst(cacheEvalRst);
            }
            tempStr = lexer.stringValue();	/* _latin1 bit 指定bit的编码格式 */
            lexer.nextToken();
        ...
        case PUNC_LEFT_PAREN:
            ...
            case POSITION: /* POSITION( xxx IN xxx) */
                lexer.nextToken();
            ...
            case SUBSTRING: /* SUBSTRING(xxx, xxx, xxx)  , SUBSTRING(xxx FROM xxx FOR xxx) */
                lexer.nextToken();
            ...
            case ROW:	/* ROW(xxx,xx,xx,xx) */
                lexer.nextToken();
                tempExprList = expressionList(new LinkedList<Expression>());
                return new RowExpression(tempExprList).setCacheEvalRst(cacheEvalRst);
            case TRIM:	/* TRIM(xx FROM xxx) RTRIM() LTRIM() */
                ...
                match(PUNC_RIGHT_PAREN);
                return new Trim(direction, tempExpr, tempExpr2).setCacheEvalRst(cacheEvalRst);
            case AVG:	/* AVG(xxx) , AVG(DISTINCT xxx) */
    ...
    private FunctionExpression ordinaryFunction(String id, String idUpper) throws SQLSyntaxErrorException {
        ...
        if (lexer.token() == PUNC_RIGHT_PAREN) {	/* no args */
            lexer.nextToken();
     ...
     * first <code>MATCH</code> has been consumed
		 * MATCH (tags) AGAINST ('旅游' IN BOOLEAN MODE) 
		 * MATCH (tags) AGAINST ('旅游' WITH QUERY EXPANSION ) 
		 * MATCH (tags) AGAINST ('旅游' IN NATURAL LANGUAGE MODE WITH QUERY EXPANSION) 
		 * MATCH (tags) AGAINST ('旅游' IN NATURAL BOOLEAN MODE)
		 * MATCH (tags) AGAINST ('旅游' IN BOOLEAN MODE) TODO
     */
     ...
     * first <code>INTERVAL</code> has been consumed
		 * INTERVAL(N, N1, N2, N3 ...) 返回N在N1,N2..中的下标 
		 * INTERVAL '01:15:30' hour_second 增加一段时间
     */
    private Expression intervalExpression() throws SQLSyntaxErrorException {
        Expression fstExpr;
        List<Expression> argList = null;
        if (lexer.token() == PUNC_LEFT_PAREN) {
            if (lexer.nextToken() == KW_SELECT) {	/* INTERVAL( SELECT xxxx, N1, N2) TODO */
                fstExpr = subQuery();
                match(PUNC_RIGHT_PAREN);
            } else {	/* 下标的情况 */
                fstExpr = expression();
