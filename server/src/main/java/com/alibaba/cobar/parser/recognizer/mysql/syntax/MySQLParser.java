...
public abstract class MySQLParser {
    ...
    private static enum SpecialIdentifier {
        GLOBAL,	/* SET GLOBAL var=value 设置全局变量 */
        LOCAL,	/* SET LOCAL var=value sql语句内部 */
        SESSION	/* SET SESSION var=value 当前会话 */
    }
     ...
     * first token must be {@link MySQLToken#SYS_VAR}
		 * 解析@@[GLOBAL.|LOCAL.|SESSION.|]varName,构建一个SysVarPrimary返回
     */
    public SysVarPrimary systemVariale() throws SQLSyntaxErrorException {
        ...
        if (si != null) {
            ...
            case LOCAL:
                ...
                match(IDENTIFIER);
                sys = new SysVarPrimary(scope, str, strUp);	/* TODO 这里local也设置成SESSION了，干嘛 */
                sys.setCacheEvalRst(cacheEvalRst);
                return sys;
            }
        }
        sys = new SysVarPrimary(scope, str, strUp);	/* 设置默认为session 变量 */
        sys.setCacheEvalRst(cacheEvalRst);
        return sys;
    }
     ...
     *         if none is matched.
		 * 返回expectTextUppercases中匹配当前identifier的下标
     */
     ...
     * @throws SQLSyntaxErrorException if no token is matched
		 * 匹配expectTextUppercase列表中是否已经有identifier
     */
     ...
     * @return index of expected token, start from 0
     * @throws SQLSyntaxErrorException if no token is matched
		 * 检查解析器接下来是否是expectToken这些token其中的一个
     */
