...
public class MySQLFunctionManager {
    public static enum FunctionParsingStrategy {
        ...
        /** ordinary function */
        _ORDINARY,
        CAST,		/* 类型转换  CAST(value AS type) */
        POSITION,	/* POSITION(substr IN str) 返回substr在str中的位置 */
        SUBSTRING,/* SUBSTRING(str, pos, lenght) 截取子串 */
        TRIM,	/* TRIM(string) 去除头尾的空格 */
        AVG,	/* AVG(字段名) 计算平均值 */
        COUNT,	/* COUNT(字段名) */
        GROUP_CONCAT,
        MAX,
        ...
        ROW,
        CHAR,
        CONVERT,	/* 类型转换 CONVERT(value, type) */
        EXTRACT,
        TIMESTAMPADD,
        TIMESTAMPDIFF,
        GET_FORMAT
