...
public class MySQLLexer {
    ...
    protected final char[] sql;	/* 传入的sql语句 */
    /** always be {@link #sql}.length - 1 */
    protected final int eofIndex;	/* sql语句末尾空格的下标 */

    /** current index of {@link #sql} */
    protected int curIndex = -1;	/* 当前处理的字符串 */
    ...
    protected boolean inCStyleComment;	/* 在mysql独有的语法中 */
    ...
    protected void scanNumber() throws SQLSyntaxErrorException {
        offsetCache = curIndex;
        sizeCache = 1;
        final boolean fstDot = ch == '.';	/* 表明第一个是不是. */
        boolean dot = fstDot;
        boolean sign = false;
        ...
        for (; !eof();) {
            ...
            case '/':
                if (hasChars(2) && '*' == sql[curIndex + 1]) { 
                    boolean commentSkip;
                    if ('!' == sql[curIndex + 2]) {	/* \/*! *\/ mysql将一些仅在mysql才有的语句放在这里，主要是mysql的版本号 */
                        scanChar(3);
                        ...
                        skipSeparator(); 	/* 接下来是mysql的语句，所以需要作为一条单独的语句处理 TODO */
                    } else {	/* \/* *\/ 这样注释类型 */
                        scanChar(2);
                        commentSkip = true;
                    }
