 ...
 *
 * 异步模式下，处理单个后端的请求 
 * 异步模式下，先客户端线程发起，然后异步起线程操作后端连接，然后selec触发后有MySQLConnectionHandler处理，等MySQLConnectionHandler判断是ok error field row包之后在调用对应ResponseHandle来处理具体事务
 */
public class SingleNodeHandler implements ResponseHandler, Terminatable {
    private final RouteResultsetNode route;		/* 路由结果 */
    private final NonBlockingSession session;	/* 对应的session */
    private byte packetId;										/* 请求的id，单次session中从0递增 */
    private volatile ByteBuffer buffer;				/* TODO */
    private ReentrantLock lock = new ReentrantLock();	/* lock */
    private boolean isRunning;								/* 是否正在处理请求 */
    private Runnable terminateCallBack;				/* TODO 中断的回调 */
    ...
    public void terminate(Runnable callback) {
        ...
        lock.lock();
        try {
            if (isRunning) {	/* 如果正在执行，那么就等执行完在调用terminate */
		...
		/* 设置idRunning=false，然后调用terminateCallBack */
    ...
    public void execute() throws Exception {
        ...
        if (conn == null) {
            ...
            dn.getConnection(this, null);	/* 和后端连接建立后，就会调用connectionAcquired，其中会异步执行_execute函数 */
        } else {
    ...
    public void connectionAcquired(final MySQLConnection conn) {
        ...
        session.getSource().getProcessor().getExecutor().execute(new Runnable() {	/* 异步执行 */
            @Override
            public void run() {
                _execute(conn);
            }
    ...
    public void errorResponse(byte[] err, MySQLConnection conn) {
		...
		/* 当接收到ok Packet的时候，由MySQLConnectiondHandler调用 */
    @Override
    public void okResponse(byte[] data, MySQLConnection conn) {
        ...
        if (executeResponse) {	/* 接收ok，则返回客户端 */
            conn.setRunning(false);
            ServerConnection source = session.getSource();
