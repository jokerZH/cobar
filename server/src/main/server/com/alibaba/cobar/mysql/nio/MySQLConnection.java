...
public class MySQLConnection extends BackendConnection {
    ...
    private MySQLConnectionPool pool;		/* 属于的连接池 */
    private long threadId;							/* mysql上的线程id */
    private HandshakePacket handshake;	/* mysql发送的auth包 */
    ...
    private final AtomicBoolean isRunning;	/* 是否在处理请求 */
    private long lastTime; // QS_TODO
    private final AtomicBoolean isQuit;
    private volatile StatusSync statusSync;	/* TODO */
    ...
    private static class StatusSync {
        private final RouteResultsetNode rrn;	/* 路由结果 */
        private final MySQLConnection conn;		/* 后端连接 */
        private CommandPacket charCmd;	/* 设置字符集的命令 	*/
        private CommandPacket isoCmd;		/* 设置隔离等级的命令 */
        private CommandPacket acCmd;		/* 设置自动提交的命令 */
        private final int charIndex;		/* 客户端的字符集 	*/ 
        private final int txIsolation;	/* 客户端的隔离等级 */
        private final boolean autocommit;/* 客户端的自动提交*/
        private volatile boolean executed;	/* 是否正在执行 */
        ...
        private Runnable updater;	/* 用于发送请求的异步线程 */
     ...
     *         before
		 * 如果需要sync，则sync请求好之后，会调用这里，来完成后续的操作
     */
