...
public class XMLSchemaLoader implements SchemaLoader {
    ...
    private final Map<String, TableRuleConfig> tableRules;	/* rule规则和名字 */
    private final Set<RuleConfig> rules;	/* 所有rule 规则的列表 */
    private final Map<String, RuleAlgorithm> functions;	/* 所有规则函数列表 */
    ...
    public XMLSchemaLoader(String schemaFile, String ruleFile) {
        ...
        ruleLoader = null;
        this.dataSources = new HashMap<String, DataSourceConfig>();	/* 后端资源列表 */
        this.dataNodes = new HashMap<String, DataNodeConfig>();	/* 后端资源的配置，如连接池属性 */
        ...
        for (int i = 0, n = list.getLength(); i < n; ++i) {
            ...
            try {
                ...
                for (int j = 0, m = locationList.getLength(); j < m; ++j) {
                    ...
                    String[] schemas = SplitUtil.split(locStr.substring(slashIndex + 1).trim(), ',', '$', '-'); /* TODO */
